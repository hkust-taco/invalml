...
sealed abstract class GeneralType:
  ...
  def show(using Scope, InvalCtx): Str
  def showDbg: Str
...
case class Wildcard(in: Type, out: Type) extends TypeArg {
  ...
  override def show(using Scope, InvalCtx): Str = in match
    case `out` => in.show
    case Bot =>
      out match
        case Top => "?"
...
sealed abstract class Type extends GeneralType with TypeArg:
  ...
  protected[invalml] def paren(using Scope, InvalCtx): Str = toBasic match
...
sealed abstract class BasicType extends Type:
  ...
  override def show(using Scope, InvalCtx): Str =
...
case class PolyFunType(args: Ls[GeneralType], ret: GeneralType, eff: Type) extends GeneralType:
  ...
  override def show(using Scope, InvalCtx): Str = s"(${args.map(_.show).mkString(", ")}) ->{${eff.show}} ${ret.show}"
